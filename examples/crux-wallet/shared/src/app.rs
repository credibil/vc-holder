...
impl crux_core::App for App {
    ...
    fn update(
        ...
        match msg {
            ...
            Event::Credential(credential_event) => match credential_event {
            ...
            Event::Issuance(issuance_event) => match issuance_event {
                    ...
                    *model = match model.issuance_offer(&encoded_offer) {
                    ...
                    let IssuanceState::Offered { offer, .. } = state.deref_mut() else {
                        return Command::event(Event::Error(
                            "expected issuance offer state".into(),
                        ));
                    };
                    let issuer_url =
                ...
                IssuanceEvent::Issuer(Ok(res)) => {
                    ...
                    *model = match model.issuer_metadata(issuer) {
                    ...
                    let background_command: Command<Effect, Event> =
                        match cred_info.background_url() {
            ...
            Event::Presentation(presentation_event) => match presentation_event {
