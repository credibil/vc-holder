...
async fn wallet_scope() {
    ...
    // credential they want to request. Here we are hardcoding the
    ...
    let (auth_request, verifier) = state
        .authorization_request(&scope, Some(REDIRECT_URI))
    ...
    let credential_response = provider.credential(request.1).await.expect("should get credentials");
